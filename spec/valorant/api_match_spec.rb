...
RSpec.describe Valorant::MatchStats, focus: true do
  let(:client) { Valorant::MatchStats.new }
  ...
  describe '#match_data', match_data: true do
    it 'takes 1 parameter' do
      expect((Valorant::MatchStats.allocate.method(:match_data).parameters.flatten - [:req]).count).to eq(1)
    end
  ...
  describe '#matches_history', matches_history: true do
    it 'takes 4 parameters' do
      expect((Valorant::MatchStats.allocate.method(:matches_history).parameters.flatten - %i[req opt
                                                                                             key]).count).to eq(4)
    end
  ...
  describe '#matches_history_by_puuid', matches_history_by_puuid: true do
    it 'takes 3 parameters' do
      expect((Valorant::MatchStats.allocate.method(:matches_history_by_puuid).parameters.flatten - %i[req opt
                                                                                                      key]).count).to eq(3)
    end
